...
async function main() {
  ...
  const sqltif = `
    ...
    UNION ALL
    SELECT 'FFM-ENT-TTD Compliance WIFI' AS kpi, sc_lokasi.witel AS lokasi, ttd_wifi.jenis AS Area, ttd_wifi.comply AS Realisasi FROM sc_lokasi LEFT JOIN ttd_wifi ON sc_lokasi.witel = ttd_wifi.regional AND ttd_wifi.tgl = '${tgl}' AND ttd_wifi.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'FFM-ENT-Provcomp Ratio  DATIN & SIP Trunk' AS kpi, sc_lokasi.witel AS lokasi, provcomp.jenis AS Area, provcomp.comply AS Realisasi FROM sc_lokasi LEFT JOIN provcomp ON sc_lokasi.witel = provcomp.regional AND provcomp.tgl = '${tgl}' AND provcomp.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'FFM-ENT-Provcomp Ratio WiFi' AS kpi, sc_lokasi.witel AS lokasi, provcomp_wifi.jenis AS Area, provcomp_wifi.comply AS Realisasi FROM sc_lokasi LEFT JOIN provcomp_wifi ON sc_lokasi.witel = provcomp_wifi.regional AND provcomp_wifi.tgl = '${tgl}' AND provcomp_wifi.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'FFM-ENT- Compliance Underspec Warranty Guarantee Non HSI' AS kpi, sc_lokasi.witel AS lokasi, unspec_warranty.jenis AS Area, unspec_warranty.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_warranty ON sc_lokasi.witel = unspec_warranty.regional AND unspec_warranty.tgl = '${tgl}' AND unspec_warranty.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ASR-WHF-Service Availability' AS kpi, sc_lokasi.witel AS lokasi, wsa_service.lokasi AS Area, wsa_service.${bln} AS Realisasi FROM sc_lokasi LEFT JOIN wsa_service ON sc_lokasi.witel = wsa_service.witel AND wsa_service.tgl = '${tgl}' AND wsa_service.lokasi IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ASR-ENT-Q gangguan - HSI' AS kpi, sc_lokasi.witel AS lokasi, q_hsi.jenis AS Area, q_hsi.comply AS Realisasi FROM sc_lokasi LEFT JOIN q_hsi ON sc_lokasi.witel = q_hsi.regional AND q_hsi.tgl = '${tgl}' AND q_hsi.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ASR-ENT-Q gangguan DATIN' AS kpi, sc_lokasi.witel AS lokasi, q_datin.jenis AS Area, q_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN q_datin ON sc_lokasi.witel = q_datin.regional AND q_datin.tgl = '${tgl}' AND q_datin.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ONM-ENT-Underspec DATIN' AS kpi, sc_lokasi.witel AS lokasi, unspec_datin.jenis AS Area, unspec_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_datin ON sc_lokasi.witel = unspec_datin.regional AND unspec_datin.tgl = '${tgl}' AND unspec_datin.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ONM-ENT-Underspec HSI EBIS' AS kpi, sc_lokasi.witel AS lokasi, unspec_hsi.jenis AS Area, unspec_hsi.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_hsi ON sc_lokasi.witel = unspec_hsi.regional AND unspec_hsi.tgl = '${tgl}' AND unspec_hsi.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
    SELECT 'ONM-ENT-% Closed SQM? EBIS Datin' AS kpi, sc_lokasi.witel AS lokasi, sqm_datin.jenis AS Area, sqm_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN sqm_datin ON sc_lokasi.witel = sqm_datin.regional AND sqm_datin.tgl = '${tgl}' AND sqm_datin.jenis IN ('tif') WHERE sc_lokasi.reg IN ('tif', 'district')
    UNION ALL
  ...
  const sqldistrict = `
      ...
      UNION ALL
      SELECT 'FFM-ENT-TTD Compliance WIFI' AS kpi, sc_lokasi.witel AS lokasi, ttd_wifi.jenis AS Area, ttd_wifi.comply AS Realisasi FROM sc_lokasi LEFT JOIN ttd_wifi ON sc_lokasi.witel = ttd_wifi.regional AND ttd_wifi.tgl = '${tgl}' AND ttd_wifi.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'FFM-ENT-Provcomp Ratio  DATIN & SIP Trunk' AS kpi, sc_lokasi.witel AS lokasi, provcomp.jenis AS Area, provcomp.comply AS Realisasi FROM sc_lokasi LEFT JOIN provcomp ON sc_lokasi.witel = provcomp.regional AND provcomp.tgl = '${tgl}' AND provcomp.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'FFM-ENT-Provcomp Ratio WiFi' AS kpi, sc_lokasi.witel AS lokasi, provcomp_wifi.jenis AS Area, provcomp_wifi.comply AS Realisasi FROM sc_lokasi LEFT JOIN provcomp_wifi ON sc_lokasi.witel = provcomp_wifi.regional AND provcomp_wifi.tgl = '${tgl}' AND provcomp_wifi.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'FFM-ENT- Compliance Underspec Warranty Guarantee Non HSI' AS kpi, sc_lokasi.witel AS lokasi, unspec_warranty.jenis AS Area, unspec_warranty.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_warranty ON sc_lokasi.witel = unspec_warranty.regional AND unspec_warranty.tgl = '${tgl}' AND unspec_warranty.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ASR-WHF-Service Availability' AS kpi, sc_lokasi.witel AS lokasi, wsa_service.lokasi AS Area, wsa_service.${bln} AS Realisasi FROM sc_lokasi LEFT JOIN wsa_service ON sc_lokasi.witel = wsa_service.witel AND wsa_service.tgl = '${tgl}' AND wsa_service.lokasi IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ASR-ENT-Q gangguan - HSI' AS kpi, sc_lokasi.witel AS lokasi, q_hsi.jenis AS Area, q_hsi.comply AS Realisasi FROM sc_lokasi LEFT JOIN q_hsi ON sc_lokasi.witel = q_hsi.regional AND q_hsi.tgl = '${tgl}' AND q_hsi.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ASR-ENT-Q gangguan DATIN' AS kpi, sc_lokasi.witel AS lokasi, q_datin.jenis AS Area, q_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN q_datin ON sc_lokasi.witel = q_datin.regional AND q_datin.tgl = '${tgl}' AND q_datin.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ONM-ENT-Underspec HSI EBIS' AS kpi, sc_lokasi.witel AS lokasi, unspec_hsi.jenis AS Area, unspec_hsi.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_hsi ON sc_lokasi.witel = unspec_hsi.regional AND unspec_hsi.tgl = '${tgl}' AND unspec_hsi.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ONM-ENT-Underspec DATIN' AS kpi, sc_lokasi.witel AS lokasi, unspec_datin.jenis AS Area, unspec_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN unspec_datin ON sc_lokasi.witel = unspec_datin.regional AND unspec_datin.tgl = '${tgl}' AND unspec_datin.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
      SELECT 'ONM-ENT-% Closed SQM? EBIS Datin' AS kpi, sc_lokasi.witel AS lokasi, sqm_datin.jenis AS Area, sqm_datin.comply AS Realisasi FROM sc_lokasi LEFT JOIN sqm_datin ON sc_lokasi.witel = sqm_datin.regional AND sqm_datin.tgl = '${tgl}' AND sqm_datin.jenis IN ('reg') WHERE sc_lokasi.reg IN ('nas', 'witel')
      UNION ALL
